...
void p2m_flush_table(struct p2m_domain *p2m)
{
    ...
    if ( p2m->root )
    {
        ...
        for ( i = 0; i < P2M_ROOT_PAGES; i++ )
        {
            ...
            /*
             * Individual altp2m views can be flushed, whilst altp2m is active.
             * To avoid inconsistencies on CPUs that continue to use the views
             * to be flushed (e.g., see altp2m_reset), we must remove every p2m
             * entry atomically.
             */
