...
int map_mmio_regions(struct domain *d,
                     ...
                     mfn_t mfn)
{
    return p2m_insert_mapping(d, p2m_get_hostp2m(d), start_gfn, nr, mfn,
...
int guest_physmap_add_entry(struct domain *d,
                            ...
                            p2m_type_t t)
{
    return p2m_insert_mapping(d, p2m_get_hostp2m(d), gfn, (1 << page_order), mfn,
                              MATTR_MEM, t);
}
    ...
    if ( mfn_eq(mfn, INVALID_MFN) )
    {
        ...
        if ( level != 3 )
        {
            rc = apply_p2m_changes(d, ap2m, INSERT, _gfn(paddr_to_pfn(gpa & mask)),
                                   nr, mfn, mattr, 0, p2mt, memaccess[xma_old]);
            if ( rc < 0 )
                goto out;
        }
