...
#undef ACCESS
    ...
    mfn = p2m_lookup_attr(ap2m, gfn, &p2mt, &level, NULL);

    /* Check host p2m if no valid entry in ap2m. */
    if ( mfn_eq(mfn, INVALID_MFN) )
    {
        /* Check if entry is part of the host p2m view. */
        mfn = p2m_lookup_attr(hp2m, gfn, &p2mt, &level, &xma_old);
        ...
        if ( level != 3 )
        {
            rc = modify_altp2m_entry(d, ap2m, gfn, mfn,
    ...
    level = 3;
    rc = modify_altp2m_entry(d, ap2m, gfn, mfn, level, p2mt, a);
...
#undef ACCESS
    ...
    /* Check if entry is part of the host p2m view. */
    mfn = p2m_lookup_attr(hp2m, gfn, &p2mt, &level, &xma);
    if ( mfn_eq(mfn, INVALID_MFN) )
        goto out;

    rc = modify_altp2m_entry(d, ap2m, gfn, mfn, level,
                             p2mt, memaccess[xma]);
    if ( rc )
    {
                            ...
                            p2m_access_t p2ma)
{
    ...
    for ( i = 0; i < MAX_ALTP2M; i++ )
    {
             ...
             (gfn_x(sgfn) <= gfn_x(p2m->max_mapped_gfn)) )
        {
            ...
            rc = modify_altp2m_range(d, p2m, sgfn, nr, smfn, mask, p2mt, p2ma);
    }
...
#undef ACCESS
    ...
    mfn = p2m_lookup_attr(ap2m, old_gfn, &p2mt, &level, NULL);
    ...
    if ( gfn_eq(new_gfn, INVALID_GFN) )
    {
        ...
        if ( !mfn_eq(mfn, INVALID_MFN) )
        {
            rc = remove_altp2m_entry(d, ap2m, old_gfn, mfn, level);
            if ( rc )
            {
                rc = -EINVAL;
                goto out;
    ...
    if ( mfn_eq(mfn, INVALID_MFN) )
    {
        mfn = p2m_lookup_attr(hp2m, old_gfn, &p2mt, &level, &xma);
        if ( mfn_eq(mfn, INVALID_MFN) ||
        ...
        if ( level != 3 )
        {
            rc = modify_altp2m_entry(d, ap2m, old_gfn, mfn,
    ...
    mfn = p2m_lookup_attr(ap2m, new_gfn, &p2mt, &level, &xma);

    /* If new_gfn is not part of altp2m, get the mapping information from hp2m */
    if ( mfn_eq(mfn, INVALID_MFN) )
        mfn = p2m_lookup_attr(hp2m, new_gfn, &p2mt, &level, &xma);
