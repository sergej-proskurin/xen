...
#undef ACCESS
    ...
    mfn = p2m_lookup_attr(ap2m, gfn, &p2mt, &level, &mattr, NULL);

    /* Check host p2m if no valid entry in ap2m. */
    if ( mfn_eq(mfn, INVALID_MFN) )
    {
        /* Check if entry is part of the host p2m view. */
        mfn = p2m_lookup_attr(hp2m, gfn, &p2mt, &level, &mattr, &xma_old);
        ...
        if ( level != 3 )
        {
            rc = modify_altp2m_entry(d, ap2m, gpa, pfn_to_paddr(mfn_x(mfn)),
                                     level, mattr, p2mt, memaccess[xma_old]);
            if ( rc < 0 )
            {
                rc = -ESRCH;
                goto out;
...
#undef ACCESS
    ...
    *ap2m = altp2m_get_altp2m(v);
    ...
    /* Check if entry is part of the host p2m view */
    mfn = p2m_lookup_attr(hp2m, gfn, &p2mt, &level, &mattr, &xma);
    if ( mfn_eq(mfn, INVALID_MFN) )
        goto out;

    rc = modify_altp2m_entry(d, *ap2m, gpa, pfn_to_paddr(mfn_x(mfn)), level,
                             mattr, p2mt, memaccess[xma]);
    if ( rc )
    {
